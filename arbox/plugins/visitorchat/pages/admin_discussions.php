...
<div class="wrap">
    ...
    <h2 style="margin-bottom: 20px;"><?php echo __('VisitorChat Past Discussions', 'visitorchat'); ?></h2>

    <?php
    global $wpdb;
    $discussionsTable = CEVC::i()->Discussion->fullTableName;
    $messagesTable = CEVC::i()->Message->fullTableName;

    $pagenum = isset($_GET['pagenum']) ? absint($_GET['pagenum']) : 1;
    $limit = 10;
    $offset = ( $pagenum - 1 ) * $limit;
    $entries = $wpdb->get_results($wpdb->prepare("SELECT Discussion.*, COUNT(Message.id) as message_count  FROM $discussionsTable AS Discussion LEFT JOIN $messagesTable AS Message ON ( Message.discussion_id = Discussion.id ) GROUP BY Discussion.id ORDER BY created DESC LIMIT %d, %d", array($offset, $limit)), ARRAY_A);
    ?>
        ...
        <table class="widefat">
            <thead>
                <tr>
                    ...
                    <th scope="col" class="manage-column column-name" style=""><?php echo __('Messages', 'visitorchat'); ?></th>
                    <th scope="col" class="manage-column column-name" style=""><?php echo __('Modified', 'visitorchat'); ?></th>
            ...
            <tbody>
                ...
                if ($entries)
                {
                    ...
                    foreach ($entries as $entry)
                    {
                        ...
                        <tr<?php echo $class; ?>>
                            ...
                            <?php endif; ?>
                            <td>
                                <a href="<?php echo admin_url('admin.php?page=visitorchat-discussions&view=' . $entry['id']) ?>" class="button action">View</a>
                            </td>
                            <td><?php echo CEVC::i()->Helper->renderVisitorname($entry['username'], $entry['email'], 16); ?></td>
                            <td><?php echo $entry['email']; ?></td>
                            <td><?php echo $entry['message_count']; ?></td>
                ...
                else
                {
                    ?>
                    <tr>
                        <td colspan="5"><?php echo __('No discussions found...', 'visitorchat'); ?></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
    ...
    <?php
    $total = $wpdb->get_var("SELECT COUNT(`id`) FROM $discussionsTable");
